...
def get_all(request):
    ...
    if request.method == "POST":
        try:
            ...
            products = ProductModel.objects.filter(
                acc__user_id=user.id, acc__value=True
            )
    ...
    data = {"error": "Необходимо отправить идентификатор пользователя"}
...
def get_all_from_product(request):
    ...
    if request.method == "POST":
        try:
            ...
            user = UserModel.objects.get(id=user_id)
            lessons = ProductModel.objects.get(
                id=product_id, acc__user_id=user.id, acc__value=True
            ).lessons
